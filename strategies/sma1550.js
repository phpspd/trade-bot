    ...
    for (let ISIN of ISINs) {
        ...
        for (let i = 366; i > 0; --i) {
            ...
            if (result.items[ISIN].graphic.length > 2) {
                ...
                if ((T1Data.sma15 - T1Data.price) / T1Data.price > 0.03 && (T1Data.ema100 - T1Data.price) / T1Data.ema100 > 0.065) {
                    buy.push({
                        ISIN: ISIN,
                        date: T0Data.date,
                        price: T0Data.price,
                ...
                for (let buyItem of buy) {
                    ...
                    if (
                        buyItem.reason == 1 && T0Data.price / buyItem.price >= 1.05 && (T1Data.sma50 / T1Data.ema100 >= 1.015 || (T1Data.ema20 - T1Data.ema100) / T1Data.ema100 > 0.05)
                    ) {
                        buyItem.soldPrice = T0Data.price;
                        buyItem.soldDate = T0Data.date;
                        buyItem.sold = true;
                    } else if (buyItem.reason == 2 && T0Data.price / buyItem.price >= 1.05 && T1Data.ema100 / T1Data.price < 0.95) {
                        buyItem.soldPrice = T0Data.price;
                        buyItem.soldDate = T0Data.date;
                        buyItem.sold = true;
                    }
